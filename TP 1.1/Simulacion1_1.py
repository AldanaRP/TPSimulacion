...
argv = sys.argv[1:]
try:
    opciones, args = getopt.getopt(argv, "c:n:e:", ["corridas=", "tiradas=", "numero="])
    ...
    print("Uso: ruleta.py -c <nro_corridas> -n <nro_tiradas> -e <nro_elegido>")
    sys.exit(1)
...
print(f"Frecuencia del número {nro_elegido}: {corridas_tot.count(nro_elegido)} veces ({(corridas_tot.count(nro_elegido) / len(corridas_tot)) * 100:.2f}%)")
...
print(f"Frecuencia relativa del número {nro_elegido}: {frec_relativa_total:.4f} ({frec_relativa_total*100:.2f}%)")
